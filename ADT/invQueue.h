...
/**
 ...
 - EmptyiQueue: -> iQueue. Generadora implementada en el
   constructor sin parámetros.
 - push_back: iQueue, Elem -> iQueue. Generadora
 - pop_front: iQueue - -> iQueue. Modificadora parcial.
 - front: iQueue - -> Elem. Observadora parcial.
 - empty: iQueue -> Bool. Observadora.
 - size: iQueue -> Entero. Observadora.
...
public:
	...
	/** Constructor; operacion EmptyiQueue */
	iQueue() : _prim(NULL), _ult(NULL), _numElems(0) {
	}
	...
	*/
	...
	/**
	 ...
	 pop_front(Push_back(elem, EmptyiQueue)) = EmptyiQueue
	 pop_front(Push_back(elem, xs)) = push_back(elem, pop_front(xs)) si !empty(xs)
	 error: pop_front(EmptyiQueue)
	*/
	void pop_front() {
		if (empty())
	...
	/**
	 ...
	 front(Push_back(elem, EmptyiQueue)) = elem
	 front(Push_back(elem, xs)) = primero(xs) si !empty(xs)
	 error: front(EmptyiQueue)
	...
	/** Constructor copia */
	iQueue(const iQueue<T> &other) : _prim(NULL), _ult(NULL) {
		copia(other);
	}

	/** Operador de asignación */
	iQueue<T> &operator=(const iQueue<T> &other) {
		if (this != &other) {
			libera();
			copia(other);
		}
	...
	bool operator==(const iQueue<T> &rhs) const {
